    ...
    describe("grantRole", function () {
      ...
      it("should not grant roles using the wrong role", async function () {
...

        {
          ...
          await Claims.connect(Signer(0)).revokeRole(Role("NOT_ROLE"), Address(9));
        }

        {
          expect(await Claims.hasRole(Role("BOT_ROLE"), Address(7))).to.equal(false); // no change
          expect(await Claims.hasRole(Role("BOT_ROLE"), Address(9))).to.equal(true);  // no change
        }
      });
    });
  });
